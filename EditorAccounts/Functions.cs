...
namespace EditorAccounts
{
    class Functions : Structs
    {
      ...
      string DirAccount = Directory.GetCurrentDirectory() + "./account/" + Account.Info.AccountName[0].ToString();
        ...
        DirAccount += "/" + Account.Info.AccountName;  // Ajuste para incluir o nome do arquivo correto

        File.WriteAllBytes(DirAccount, data);

        // Mensagem de sucesso após salvar os dados no arquivo
        MessageBox.Show("Os dados foram salvos com sucesso no arquivo: " + DirAccount, "Salvamento Concluído", MessageBoxButtons.OK, MessageBoxIcon.Information);
    }
    catch (Exception ex)
    {
        MessageBox.Show("Falha ao salvar os dados: " + ex.Message, "Erro ao Salvar", MessageBoxButtons.OK, MessageBoxIcon.Error);
    }
        }
