...
namespace EditorAccounts
{
    public partial class Form1 : Form
    {
        ...
        public void UpdateInfo()
        {
            ...
            for (int i = 0; i < External.g_pAcccount.Char.Length; i++) // Usando .Length para maior flexibilidade
            {
                // Verifica se o nome do personagem é válido
                if (!string.IsNullOrEmpty(External.g_pAcccount.Char[i].name))
                {
                    // Adiciona o nome válido ao controle de interface do usuário
        ...
        private void abrirContaToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                currentFilePath = openFileDialog1.FileName; // Armazena o caminho do arquivo aberto

                try
                {

                    // Código existente para abrir o arquivo e carregar dados
                    Functions.LoadAccount(currentFilePath);
                    UpdateInfo();
        ...
        private void txt_Leave(object sender, EventArgs e)
        {
            ...
            // Garantir que o índice selecionado está dentro dos limites do array
            if (External.CurrentChar < 0 || External.CurrentChar >= External.g_pAcccount.Char.Length)
            {
                MessageBox.Show("Índice de personagem inválido.");
                return; // Encerra a execução do método para evitar o erro
            }

            try
            {
                // Atualiza as propriedades do personagem com os valores dos TextBoxes
                External.g_pAcccount.Char[External.CurrentChar].name = txtNome.Text;
                External.g_pAcccount.Char[External.CurrentChar].BaseScore.Level = Convert.ToInt32(txtLevel.Text);
                External.g_pAcccount.Char[External.CurrentChar].Exp = Convert.ToInt64(txtExp.Text);
                External.g_pAcccount.Char[External.CurrentChar].Coin = Convert.ToInt32(txtCoin.Text);
        ...
        private void SaveChanges()
        {
            ...
            {
                MessageBox.Show("Nenhum arquivo foi carregado para salvar as alterações.");
                return;
            }
            try
            {
                // Aqui você passa apenas o objeto da conta para a função SaveAccount.
                Functions.SaveAccount(External.g_pAcccount);
                labelSave.Text = string.Format("Conta {0} salva com sucesso!", External.g_pAcccount.Info.AccountName);
            }
            catch (Exception ex)
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            string folderPath = @"C:\W2-ACCOUNT-EDITOR\Build\DBSrv\run\account\";

            // Código para salvar seus dados aqui
            // ...

            // Mostrar uma mensagem de sucesso
          //   MessageBox.Show("Os dados foram salvos com sucesso na pasta:\n" + folderPath, "Salvamento Concluído", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            try
            {
                if (Directory.Exists(folderPath))
                {
                    ...
                    {
                        FileName = folderPath,
                        UseShellExecute = true // Necessário para abrir a pasta no Windows Explorer
                    });
                }
                else
                {
                    MessageBox.Show("Pasta não encontrada: " + folderPath, "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
            {
